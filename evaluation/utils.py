...
TREE_SITTER_ROOT = Path(__file__).parents[1].joinpath("third_party")
...
def run_cpp_program(script_path, i):
    folder = os.path.dirname(script_path)
    name = os.path.basename(script_path).split(".")[0]
    # Added -std=c++11 to work on our environment
    proc = subprocess.Popen(
        f"{limit_virtual_memory(MAX_VIRTUAL_MEMORY)}; "
        f"cd {folder} && g++ -std=c++11 {name}.cpp -o {name}_cpp && ./{name}_cpp",
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        shell=True,
        executable="/bin/bash",
